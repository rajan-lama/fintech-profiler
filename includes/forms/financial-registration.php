 ...
 */
add_action('template_redirect', 'financial_handle_frontend_actions');
function financial_handle_frontend_actions()
{
  ...
  if ($action === 'reset_request') {
    financial_process_reset_request();
  }
}
...
//     financial_redirect_with_args(array('fp_login' => 'failed', 'fp_msg' => 'Invalid request.'));
//   }

//   if (fp_rate_limit_exceeded()) {
//     financial_redirect_with_args(array('fp_login' => 'failed', 'fp_msg' => 'Too many attempts. Try again later.'));
//   }
...
//       financial_redirect_with_args(array('fp_login' => 'failed', 'fp_msg' => 'reCAPTCHA verification failed.'), $redirect);
//     }
//   }

//   if ('' === $user_login || '' === $user_pass) {
//     fp_increment_failed_attempts();
//     financial_redirect_with_args(array('fp_login' => 'failed', 'fp_msg' => 'Missing username or password.'), $redirect);
//   }
...
//   if (is_wp_error($user)) {
//     fp_increment_failed_attempts();
//     financial_redirect_with_args(array('fp_login' => 'failed', 'fp_msg' => fp_first_error_message($user)), $redirect);
//   }

//   // successful
//   fp_clear_failed_attempts();
//   financial_process_reset_request($user);
//   wp_set_current_user($user->ID);
//   wp_safe_redirect($redirect ? $redirect : home_url(fp_DEFAULT_REDIRECT));
//   exit;
// }

function financial_generate_unique_username_from_email($email)
{
  $parts = explode('@', $email);
 ...
 */
function financial_process_register()
{
  if (! isset($_POST['fp_nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['fp_nonce'])), 'fp_register_nonce')) {
    financial_redirect_with_args(array('fp_reg_msg' => 'Invalid request.'));
  }

  if (fp_rate_limit_exceeded()) {
    financial_redirect_with_args(array('fp_reg_msg' => 'Too many attempts. Try again later.'));
  }
  ...
  if (fp_RECAPTCHA_ENABLED) {
    ...
    if (! fp_verify_recaptcha($token)) {
      fp_increment_failed_attempts();
      financial_redirect_with_args(array('fp_reg_msg' => 'reCAPTCHA verification failed.'));
    }
  }

  $username = isset($_POST['fp_reg_user']) ? sanitize_user(wp_unslash($_POST['fp_reg_user']), true) : financial_generate_unique_username_from_email(isset($_POST['fp_reg_email']) ? sanitize_email(wp_unslash($_POST['fp_reg_email'])) : '');
  ...
  if ($errors->has_errors()) {
    fp_increment_failed_attempts();
    financial_redirect_with_args(array('fp_reg_msg' => fp_first_error_message($errors)));
  }
  ...
  if (is_wp_error($user_id)) {
    fp_increment_failed_attempts();
    financial_redirect_with_args(array('fp_reg_msg' => fp_first_error_message($user_id)));
  }
 ...
 */
function financial_process_reset_request()
{
  if (! isset($_POST['fp_nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['fp_nonce'])), 'fp_reset_nonce')) {
    financial_redirect_with_args(array('fp_reset_msg' => 'Invalid request.'));
  }

  if (fp_rate_limit_exceeded()) {
    financial_redirect_with_args(array('fp_reset_msg' => 'Too many attempts. Try again later.'));
  }
  ...
  if (fp_RECAPTCHA_ENABLED) {
    ...
    if (! fp_verify_recaptcha($token)) {
      fp_increment_failed_attempts();
      financial_redirect_with_args(array('fp_reset_msg' => 'reCAPTCHA verification failed.'));
    }
  }
  ...
  if (empty($user_input)) {
    fp_increment_failed_attempts();
    financial_redirect_with_args(array('fp_reset_msg' => 'Please provide username or email.'));
  }
  ...
  if (! $user) {
    ...
    financial_redirect_with_args(array('fp_reset_msg' => 'If the account exists, you will receive an email with reset instructions.'));
  }

  // Use WP core functions to generate reset key and send email
  $allow = apply_filters('allow_password_reset', true, $user->user_login);
  if (! $allow) {
    financial_redirect_with_args(array('fp_reset_msg' => 'Password reset is not allowed for this account.'));
  }

  $key = get_password_reset_key($user);
  if (is_wp_error($key)) {
    financial_redirect_with_args(array('fp_reset_msg' => 'Could not generate reset key.'));
  }
 ...
 */
// function financial_process_reset_request($user)
// {
 ...
 */
function financial_redirect_with_args(array $args, $fallback = '')
{
