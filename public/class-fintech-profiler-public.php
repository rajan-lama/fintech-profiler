...
class Fintech_Profiler_Public
{
	...
	public function enqueue_styles()
	{
		...
		wp_enqueue_script('boostrap', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css', array(), '4.7.0', 'all');
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script('boostrap', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js', array('jquery'), $this->version, false);
		...
		$fintech_id	= (!empty($_GET["fintech"])) ? $_GET["fintech"] : get_the_ID();
		...
		wp_localize_script($this->plugin_name, 'fintech_ajax', array(
			...
			'country' => get_post_meta($fintech_id, 'fintech_country', true),
			'state' => get_post_meta($fintech_id, 'fintech_state', true),
		));

		// wp_enqueue_script('fp-media-uploader', get_stylesheet_directory_uri() . '/js/compan.js', ['jquery', 'jquery-ui-sortable'], null, true);

		// wp_localize_script('fp-media-uploader', 'ajaxurl', admin_url('admin-ajax.php'));
		// wp_localize_script('fp-media-uploader', 'fp_media', array(
		// 	'nonce' => wp_create_nonce('fp_media_uploader_nonce'),
		// ));

		wp_enqueue_script('fp-media-uploader', FINTECH_PROFILER_BASE_URL . '/public/js/company-logo-upload.js', ['jquery', 'jquery-ui-sortable'], null, true);

		wp_localize_script('fp-media-uploader', 'fp_media', [
